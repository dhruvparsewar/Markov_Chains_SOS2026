...
\section{\textcolor{secondary}{Problem Statement}}
The objective of this implementation is to demonstrate how a model-free Reinforcement Learning agent which is using Q-Learning adapts its policy when navigating an environment with uncertainty.

In classic deterministic pathfinding algorithms, an agent assumes perfect execution of its actions. However, real-world environments are mainly stochastic.

We aim to model this uncertainty using a Markov Decision Process (MDP) in a $5 \times 5$ Gridworld. The environment contains a highly rewarding goal state and ``pit'' states which reward negatively. By manipulating the ``slip probability'' the chance that an agent moves perpendicularly to its intended direction, we aim to observe how the optimal policy transitions from the standard shortest-path strategy to a cautious, risk-averse strategy.

\vspace{0.5cm}
\subsection{\textcolor{secondary}{Proposed Solution}}
...
\vspace{0.5cm}
\subsection{\textcolor{secondary}{Code Implementation \& Markov Chain Integration}}
...
\newpage
\subsection{\textcolor{secondary}{Results and Analysis}}
